...
//  ----------------------------------------------------------
    ...
    //  ---------------------- Doxygen info ----------------------
//! \fn RMLPositionInputParameters(const RMLInputParameters& IP)
//! 
//! \brief
//! Copy constructor of class RMLPositionInputParameters
//! 
...
//! Object to be copied
//  ----------------------------------------------------------
        RMLPositionInputParameters(const RMLInputParameters &IP) : RMLInputParameters(IP)
    {
        this->MaxVelocityVector						=	new RMLDoubleVector	((IP.CurrentPositionVector)->GetVecDim())	;
        this->TargetPositionVector					=	new RMLDoubleVector	((IP.CurrentPositionVector)->GetVecDim())	;
        this->AlternativeTargetVelocityVector		=	new RMLDoubleVector	((IP.CurrentPositionVector)->GetVecDim())	;
        
        memset(this->MaxVelocityVector->VecData					,	0x0	,		IP.NumberOfDOFs * sizeof(double));
        memset(this->TargetPositionVector->VecData				,	0x0	,		IP.NumberOfDOFs * sizeof(double));
        memset(this->AlternativeTargetVelocityVector->VecData	,	0x0	,		IP.NumberOfDOFs * sizeof(double));
    }
...
//! \sa \ref page_InputValues
//  ----------------------------------------------------------
    bool CheckForValidity(void) const override
    {
        unsigned int		i							=	0;
