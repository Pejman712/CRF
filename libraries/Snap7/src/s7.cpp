//******************************************************************************************************
// Copyright. This library is comlement of snap7 library to do mapping of buffer and define types
//
// S7 Library allow to mapping the buffer into the different S7 data types based on wrapper made
// for .Net and tested for C++. It defines other types such us PLCs, Area Sources
//
...
std::string S7_GetTxtPLCType(short int plcType) {  // NOLINT
  switch (plcType) {
    case S7_PLC_300_400:
        return "S7 300/400";
        break;
...
//****************************************************************************
// Get the word from Transport Service Access Point (TSAP) in hex format, e.g: 10.02 => 0x1002,
// used by Cli_SetConnectionParams. Enter TSAP in format ##.## in hex
// Return: the TSAP number in a word and decimal format
uint16_t S7_GetWordFromTSAP(std::string TSAP) {
  short int pos = TSAP.find('.');  // find the delimiter into the source string NOLINT
  ...
  if (pos == std::string::npos) {  // no delimiter was found, return 0
    ...
    uint8_t number1 = strtol(first.c_str(), &p, 16);  // convert string with data in any base (10, 16) to long NOLINT
    uint8_t number2 = strtol(second.c_str(), &p, 16);  // convert string with data in any base (10, 16) to long NOLINT
...
int32_t S7_GetDIntAt(byte Buffer[], int Pos) {
  int32_t Result;
...
void S7_SetDIntAt(byte Buffer[], int Pos, int32_t Value) {
...
std::string S7_GetStringAt(byte Buffer[], int Pos) {
  std::string res;
  int size = (int)Buffer[Pos + 1];  // NOLINT
  res.insert(0, (char*)&Buffer[Pos+2], size);  // NOLINT
  return res;
}
...
void S7_SetStringAt(byte Buffer[], int Pos, int MaxLen, std::string Value) {
  ...
  Value.copy((char*)&Buffer[Pos+2], size);  // NOLINT
}
...
std::string S7_GetCharsAt(byte Buffer[], int Pos, int Size) {
  std::string res;
  res.insert(0, (char*)&Buffer[Pos], Size);  // NOLINT
  return res;
}
...
void S7_SetCharsAt(byte Buffer[], int BufferLen, int Pos, std::string Value) {
  ...
  Value.copy((char*)&Buffer[Pos], Size);  // NOLINT
}
