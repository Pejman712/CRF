...
CHARMBot::CHARMBot(std::shared_ptr<communication::cansocket::ICANSocket> socket,
    const nlohmann::json& configuration) :
        logger_("CHARMBot"),
        initialized_(false),
        radToRPMConverter_(60/(2*M_PI)),
        motorsDirection_({1, -1, 1, -1}),
        gearBoxReduction_(26*1.54),
        socket_(socket),
        motors_(),
        ...
        maximumWheelsAcceleration_(0) {
            ...
            auto motorFrontLeft = std::make_shared<devices::canopendevices::MaxonEPOS4>(0x02, socket_); // NOLINT
            auto motorFrontRight = std::make_shared<devices::canopendevices::MaxonEPOS4>(0x01, socket_); // NOLINT
            auto motorBackLeft = std::make_shared<devices::canopendevices::MaxonEPOS4>(0x03, socket_); // NOLINT
            auto motorBackRight = std::make_shared<devices::canopendevices::MaxonEPOS4>(0x04, socket_); // NOLINT
...
CHARMBot::CHARMBot(const nlohmann::json& configuration,
...
bool CHARMBot::initialize() {
    ...
    // Maxon EPOS4 are slightly slower than the EPOS2 to pass in ReadyToSwitchOn state
...
bool CHARMBot::deinitialize() {
    ...
    if (!ctx_->deinitialize()) {
        logger_->error("Denitialization of CANOpen context failed");
        return false;
    }
    initialized_ = false;
...
boost::optional<crf::utility::types::TaskPose> CHARMBot::getTaskPose() {
    return boost::none;
}

boost::optional<crf::utility::types::TaskVelocity> CHARMBot::getTaskVelocity() {
    if (!initialized_) {
        logger_->warn("Not initialized");
        return boost::none;
    }
...
boost::optional<std::vector<float> > CHARMBot::getMotorsVelocities() {
    if (!initialized_) {
        logger_->warn("Not initialized");
        return boost::none;
    }
...
boost::optional<std::vector<float> > CHARMBot::getMotorsCurrent() {
    if (!initialized_) {
        logger_->warn("Not initialized");
        return boost::none;
    }
...
bool CHARMBot::setTaskVelocity(const crf::utility::types::TaskVelocity& velocity) {
    if (!initialized_) {
        logger_->warn("Not initialized");
        return false;
    }
...
bool CHARMBot::setWheelsVelocity(const std::vector<float> velocity) {
     if (!initialized_) {
        logger_->warn("Not initialized");
        return false;
    }
...
bool CHARMBot::acknowledgeError() {
...
std::shared_ptr<RobotBaseConfiguration> CHARMBot::getConfiguration() {
    logger_->debug("getConfiguration");
    return configuration_;
}

bool CHARMBot::setMaximumWheelsAcceleration(float acceleration)  {
...
bool CHARMBot::setMaximumWheelsVelocity(float velocity)  {
...
bool CHARMBot::setMaximumTaskVelocity(const utility::types::TaskVelocity& velocity)  {
...
float CHARMBot::getMaximumWheelsAcceleration() const {
    return maximumWheelsAcceleration_;
}

float CHARMBot::getMaximumWheelsVelocity() const {
    return maximumWheelsVelocity_;
}

utility::types::TaskVelocity CHARMBot::getMaximumTaskVelocity() const {
