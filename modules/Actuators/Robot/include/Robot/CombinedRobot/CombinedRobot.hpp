...
class CombinedRobot: public IRobot {
 public:
    CombinedRobot() = delete;
    CombinedRobot(std::vector<std::shared_ptr<IRobot>> robots,
        const CombinedRobotConfiguration& configuration);
    ~CombinedRobot() override;
 ...
 private:
    ...
    CombinedRobotConfiguration robotConfiguration_;

    bool isInitialized_;
    uint64_t nRobots_;
    uint64_t combinedDof_;
    std::vector<uint64_t> robotsDof_;
