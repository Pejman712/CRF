...
int main(int argc, char* argv[]) {
    crf::utility::logger::EventLogger logger_("testCavityMotorInPosAbsolute");
    ...
    std::shared_ptr<crf::actuators::srfcavityManager::EtherCATSRFCavityManager> cavityManager;
    ...
    ECManager_ = std::make_shared<crf::devices::ethercatdevices::EtherCATManager>
    ...
    logger_->critical("cav pos = {} [deg]", rad2deg(cavityManager->getPosition().value()));
    if (!cavityManager->enableMotor()) {
        logger_->error("Impossible to enableMotor");
        return false;
    }

    logger_->info("cav pos = {} [deg]", rad2deg(cavityManager->getPosition()));
    bool moveToOrigin = false;
    std::string nbrDegStr = "";
    float nbrRad = 0;

    while (true) {
        std::cout << "Go to origin ?" << std::endl;
        std::cin >> moveToOrigin;

        if (moveToOrigin) {
            if (!cavityManager->moveToOrigin()) {
                logger_->critical("Impossible to go to origin. Cav pos = {} [deg]",
                    rad2deg(cavityManager->getPosition()));
            }
            logger_->info("cav pos = {} [deg]", rad2deg(cavityManager->getPosition().value()));
            logger_->info("cav pos = {} [rad]", cavityManager->getPosition().value());
        }

        std::cout << "Go to absolute value of [deg]?" << std::endl;
        std::cin >> nbrDegStr;

        pos = cavityManager->getPosition();
        logger_->info("currentPos = {} [deg]", rad2deg(pos.value()));
        nbrRad = deg2rad(std::stof(nbrDegStr));
        // nbrRad = deg2rad(std::stof(nbrDegStr));
        cavityManager->setPosition(nbrRad, true);
        pos = cavityManager->getPosition();
        logger_->info("currentPos = {} [deg]", rad2deg(pos.value()));
    }
    return 0;
