...
int main(int argc, char* argv[]) {
    ...
    // Admittance with inverse Velocity
    while (true) {
        std::this_thread::sleep_for(std::chrono::milliseconds(5));
        TaskVelocity cv{};
        TaskPose cp = ctrl->getTaskPose();
        std::cout << "Target Task" << cp << std::endl;

        TaskForceTorque ct = ftsensor->getFTGravityFree(cp, true);
        ...
        JointVelocities jv = kinematics->getVelocityInverseKinematic(
            cv,
            ctrl->getJointPositions()).get();
        JointPositions target = ctrl->getJointPositions();
        for (int j = 0; j < 6; j++) {
            target(j) += jv(j);
        }
        ctrl->setJointPositions(target);

        std::cout << "Solution Task" << cp << std::endl;
        std::cout << std::endl;
        std::cout << std::endl;
    }
