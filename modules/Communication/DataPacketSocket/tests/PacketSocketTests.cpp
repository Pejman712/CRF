...
class PacketSocketShould : public ::testing::Test {
 protected:
    ...
    void putToReadPacket(const crf::communication::datapackets::IPacket& packet,
                         const crf::communication::datapackets::PacketHeader& header) {
        bufferToRead_.clear();
        bufferToRead_.append("~~~~~~");
        bufferToRead_.append(header.serialize());
...
TEST_F(PacketSocketShould, readCorrectBytes) {
    ...
    putToReadPacket(json, json.getHeader());
    auto retval = sut_->read();

    ASSERT_TRUE(retval);
    ASSERT_EQ(retval.value().first.length(), json.serialize().length());
}

TEST_F(PacketSocketShould, HaveTheCorrectWriterIDinTheHeader) {
    sut_.reset(new PacketSocket(socket_));
    crf::communication::datapackets::JSONPacket json;
    json.data["data"] = "tentative";

    crf::communication::datapackets::PacketHeader header = json.getHeader();
    header.writerID(12);
    putToReadPacket(json, header);

    auto retval = sut_->read();
    ASSERT_TRUE(retval);
    ASSERT_EQ(retval.value().first.length(), json.serialize().length());
    ASSERT_EQ(retval.value().second.writerID(), 12);
}
