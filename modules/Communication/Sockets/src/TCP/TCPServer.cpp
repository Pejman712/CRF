...
bool TCPServer::open() {
    ...
    if (setsockopt(serverFd_, SOL_SOCKET, SO_REUSEADDR, &sl, sizeof(sl)) != 0) {
        logger_->warn("Could not set SO_REUSEADDRE flag: {}", strerror(errno));
        return false;
    }
...
std::optional<std::shared_ptr<ISocket> > TCPServer::acceptConnection() {
