...
std::string TCPSocket::blockingRead(int length) {
    ...
    // Whilst we don't expect the <=0 or the -1.
    // The check is still in place for reasurrance should it occur.
    if (retval <= 0) {
        if (retval == 0) {
            logger_->info("Peer closed socket, finishing connection.");
            close();
        }
        if (retval == -1) logger_->warn("Error during blocking read: {}", strerror(errno));
        delete[] cbuf;
        return std::string();
    }
