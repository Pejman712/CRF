...
class IClosedLoopInverseKinematics {
 public:
    virtual ~IClosedLoopInverseKinematics() = default;

    /**
     * @brief Function to get the joint positions, velocities and accelerations of the robot arm
     *        knowing the end-effector position, velocity and acceleration.
     * 
    ...
    /**
     * @brief Function to set a new qInitial.
     * 
     * @param qActual is the current robot joints configuration that is going to be used as qInitial
     *        in the next getResults or getExtendedResults call.
     */
    virtual void updateInitialJointPositions(const JointPositions& qActual) = 0;
};
