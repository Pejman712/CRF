...
int main(int argc, char* argv[]) {
    ...
    std::unique_ptr<crf::devices::ethercatdevices::TIMRobotArmWagonMotors> motors =
        std::make_unique<crf::devices::ethercatdevices::TIMRobotArmWagonMotors>(
    ...
    temp = motors->getHarmonicDrive1();
    if (!temp) {
        std::cout << "Cannot retrieve Harmonic Drive 1" << std::endl;
        return -1;
    }
    std::shared_ptr<crf::devices::ethercatdevices::IEtherCATMotor> HD1 = temp.value();

    temp = motors->getHarmonicDrive2();
    if (!temp) {
        std::cout << "Cannot retrieve Harmonic Drive 2" << std::endl;
        return -1;
    }
    std::shared_ptr<crf::devices::ethercatdevices::IEtherCATMotor> HD2 = temp.value();

    temp = motors->getLinearSled();
    if (!temp) {
        std::cout << "Cannot retrieve Linear Sled" << std::endl;
        return -1;
    }
    std::shared_ptr<crf::devices::ethercatdevices::IEtherCATMotor> Sled = temp.value();

    std::ifstream robotData(vm["configuration"].as<std::string>());
    nlohmann::json robotJSON = nlohmann::json::parse(robotData);

    auto arm = std::make_shared<crf::actuators::ethercatrobotarm::EtherCATRobotArm>(robotJSON, HD1,
        HD2, Sled);
    if (!arm->initialize()) {
