...
#include "CANopenDrivers/CiA402/CoEDrivers/ELMOPlatinumCoEDriver/ELMOPlatinumCoEDriver.hpp"

using crf::devices::canopendrivers::CoEMaster;
using crf::devices::canopendrivers::ELMOPlatinumCoEDriver;
...
int main(int argc, char *argv[]) {
    ...
    std::shared_ptr<CoEMaster> master =
        std::make_shared<CoEMaster>(ifname, 1, std::chrono::milliseconds(3), config, 4096);
    ...
    ELMOPlatinumCoEDriver driver(master, 1, config);
