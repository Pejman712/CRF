...
class ISoemApi {
 public:
     ...
     * @param usatable = TRUE when using configtable to init slaves, FALSE otherwise (we use
     *        FALSE).
    ...
    /*
     * @brief Map all PDOs from slaves to IOmap with Outputs/Inputs in sequential order.
     * @param pIOmap = pointer to IOmap.
     * @return IOmap size.
     */
    virtual int config_map(void* pIOmap) = 0;
    /*
     ...
     */
    virtual int SDOread(uint16_t slave, uint16_t index, uint8_t subindex, bool CA, int *psize,
        void *p, int timeout) = 0;
    /*
     ...
     * @return Workcounter from last slave response.
     */
    virtual int SDOwrite(uint16_t Slave, uint16_t Index, uint8_t SubIndex, bool CA, int psize,
        void *p, int Timeout) = 0;
    /*
     ...
     * @param slave = Slave number, 0 = master.
     * @return 0.
     */
