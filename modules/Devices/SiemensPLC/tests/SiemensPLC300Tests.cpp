...
/*
    Please configure the S7-300 PLC:
    IP address: 128.141.94.118, rack 0, slot 2
...
class SiemensPLC300Should: public ::testing::Test {
 protected:
    SiemensPLC300Should() :
        logger_("SiemensPLC300Should"),
        ipAddress_("128.141.94.118"),
        rack_(0),
        slot_(2),
        dbNumber_(2),
...
TEST_F(SiemensPLC300Should, DISABLED_HardwareWriteReadCompareBools) {
    ASSERT_TRUE(plc_.initialize());
    bool bools[8] = {true, false, false, true, true, true, true, false};
    for (unsigned int i = 0; i < 7; ++i) {
        plc_.writeRegister(crf::devices::siemensplc::RegisterType::R_BOOL, bools[i],
            dbNumber_, 0, i);
        auto valueBool = plc_.readRegister(crf::devices::siemensplc::RegisterType::R_BOOL,
            dbNumber_, 0, i);
        EXPECT_FALSE(isDifferent(bools[i], boost::any_cast<bool>(valueBool)));
    }
}

TEST_F(SiemensPLC300Should, DISABLED_HardwareWriteReadCompareByte) {
    ASSERT_TRUE(plc_.initialize());
    EXPECT_TRUE(writeReadCompare<uint8_t>(crf::devices::siemensplc::RegisterType::R_BYTE,
        0b11100101, dbNumber_, 1));
}

TEST_F(SiemensPLC300Should, DISABLED_HardwareWriteReadCompareWord) {
    ASSERT_TRUE(plc_.initialize());
    EXPECT_TRUE(writeReadCompare<uint16_t>(crf::devices::siemensplc::RegisterType::R_WORD,
        0b0111010101110001, dbNumber_, 2));
}

TEST_F(SiemensPLC300Should, DISABLED_HardwareWriteReadCompareInt) {
    ASSERT_TRUE(plc_.initialize());
    EXPECT_TRUE(writeReadCompare<int16_t>(crf::devices::siemensplc::RegisterType::R_INT,
        -558, dbNumber_, 4));
}

TEST_F(SiemensPLC300Should, DISABLED_HardwareWriteReadCompareDWord) {
    ASSERT_TRUE(plc_.initialize());
    EXPECT_TRUE(writeReadCompare<uint32_t>(crf::devices::siemensplc::RegisterType::R_DWORD,
        0b01110101011100010110010101100101, dbNumber_, 6));
}

TEST_F(SiemensPLC300Should, DISABLED_HardwareWriteReadCompareDInt) {
    ASSERT_TRUE(plc_.initialize());
    EXPECT_TRUE(writeReadCompare<int32_t>(crf::devices::siemensplc::RegisterType::R_DINT,
        -2556, dbNumber_, 10));
}

TEST_F(SiemensPLC300Should, DISABLED_HardwareWriteReadCompareReal) {
    ASSERT_TRUE(plc_.initialize());
    EXPECT_TRUE(writeReadCompare<float>(crf::devices::siemensplc::RegisterType::R_REAL,
        546.3434, dbNumber_, 14));
}

TEST_F(SiemensPLC300Should, DISABLED_HardwareCorrectlyReadFromDb) {
    ASSERT_TRUE(plc_.initialize());
