...
class MSDR1BilinearTransformIIRShould: public ::testing::Test {
 protected:
    MSDR1BilinearTransformIIRShould():
      logger_("MSDR1BilinearTransformIIRShould") {
        logger_->info("{} BEGIN",
    ...
    std::unique_ptr<MSDR1BilinearTransformIIR> msd_;
};

TEST_F(MSDR1BilinearTransformIIRShould, failIfParametersAreZero) {
    double Tstep = 0.01;
    double m = 0;
    double d = 0;
    double k = 0;
    double force = 1;
    msd_.reset(new MSDR1BilinearTransformIIR(Tstep));
    EXPECT_THROW(msd_->calculate(force, m, d, k), std::invalid_argument);
}

TEST_F(MSDR1BilinearTransformIIRShould, failIfAnyParameterIsNegative) {
    double Tstep = 0.01;
    double m = 1;
    double d = 1;
    double k = -1;
...
TEST_F(MSDR1BilinearTransformIIRShould, failIfTimeStepIsZeroOrNegative) {
    double Tstep = 0;
    EXPECT_THROW(msd_.reset(new MSDR1BilinearTransformIIR(Tstep)), std::invalid_argument);

    Tstep = -0.5;
    EXPECT_THROW(msd_.reset(new MSDR1BilinearTransformIIR(Tstep)), std::invalid_argument);
}

TEST_F(MSDR1BilinearTransformIIRShould, normalWorking) {
    double Tstep = 0.01;
    double m = 0.5;
    double d = 2;
    double k = 1;
    ...
    EXPECT_NEAR(signal.position, 0.09999990825, margin);
    ...
    EXPECT_NEAR(signal.position, 0.05062450385, margin);
