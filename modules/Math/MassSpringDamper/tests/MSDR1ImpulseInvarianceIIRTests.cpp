...
class MSDR1ImpulseInvarianceIIRShould: public ::testing::Test {
 protected:
    MSDR1ImpulseInvarianceIIRShould():
      logger_("MSDR1ImpulseInvarianceIIRShould") {
        logger_->info("{} BEGIN",
    ...
    std::unique_ptr<MSDR1ImpulseInvarianceIIR> msd_;
};

TEST_F(MSDR1ImpulseInvarianceIIRShould, failIfParametersAreZero) {
    double Tstep = 0.01;
    double m = 0;
    double d = 0;
    double k = 0;
    double force = 1;
    msd_.reset(new MSDR1ImpulseInvarianceIIR(Tstep));
    EXPECT_THROW(msd_->calculate(force, m, d, k), std::invalid_argument);
}

TEST_F(MSDR1ImpulseInvarianceIIRShould, failIfAnyParameterIsNegative) {
    double Tstep = 0.01;
    double m = 1;
    double d = 1;
    double k = -1;
...
TEST_F(MSDR1ImpulseInvarianceIIRShould, failIfTimeStepIsZeroOrNegative) {
    double Tstep = 0;
    EXPECT_THROW(msd_.reset(new MSDR1ImpulseInvarianceIIR(Tstep)), std::invalid_argument);

    Tstep = -0.5;
    EXPECT_THROW(msd_.reset(new MSDR1ImpulseInvarianceIIR(Tstep)), std::invalid_argument);
}

TEST_F(MSDR1ImpulseInvarianceIIRShould, normalWorking) {
    double Tstep = 0.01;
    double m = 0.5;
    double d = 2;
    double k = 1;
    ...
    EXPECT_NEAR(signal.position, 0.04999925274, margin);
    ...
    EXPECT_NEAR(signal.position, 0.05062653150, margin);
