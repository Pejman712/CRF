...
int main(int argc, char* argv[]) {
    ...
    std::cout << "[DEBUG] Initializing PCL Visualizer." << std::endl;
    pcl::visualization::PCLVisualizer::Ptr viewer(new pcl::visualization::PCLVisualizer("SLAM Map Visualization"));
    viewer->setBackgroundColor(0, 0, 0);
    viewer->addCoordinateSystem(1.0);

    // Create point cloud objects to store transformed point clouds for each method
    pcl::PointCloud<pcl::PointXYZI>::Ptr gicpTransformedCloud(new pcl::PointCloud<pcl::PointXYZI>());
    pcl::PointCloud<pcl::PointXYZI>::Ptr kalmanTransformedCloud(new pcl::PointCloud<pcl::PointXYZI>());
    pcl::PointCloud<pcl::PointXYZI>::Ptr graphTransformedCloud(new pcl::PointCloud<pcl::PointXYZI>());
    ...
    while (!ctrl_c_pressed && !viewer->wasStopped()) {
        ...
        *accumulatedCloud += *cloud;
        ...
        if (currentTime - lastSaveTime >= accumulationInterval) {
            ...
            *previousCloud = *xyz_cloud;  // Save the current scan as the previous scan
            ...
            float y_translation = cumulativeTransformation(1, 3);
            float psi = atan2(cumulativeTransformation(1,0), cumulativeTransformation(0,0));
            std::cout << "Gicp/odom cumulative: x_translation = " << x_translation
            ...
            //std::cout << "[DEBUG] Performing Kalman Filter correction." << std::ensdl;
            ...
            // Transform the target cloud using cumulativeTransformation (GICP result)
            pcl::PointCloud<pcl::PointXYZI>::Ptr transformedTargetCloudGICP(new pcl::PointCloud<pcl::PointXYZI>());
            pcl::transformPointCloud(*xyz_cloud, *transformedTargetCloudGICP, cumulativeTransformation);

            // Transform the target cloud using Kalman Filter estimated pose
            Eigen::Matrix4f kalmanTransformation = Eigen::Matrix4f::Identity();
            kalmanTransformation.block<2,2>(0,0) = Eigen::Rotation2Df(meanf[2]).toRotationMatrix();
            kalmanTransformation(0,3) = meanf[0];
            kalmanTransformation(1,3) = meanf[1];

            pcl::PointCloud<pcl::PointXYZI>::Ptr transformedTargetCloudKalman(new pcl::PointCloud<pcl::PointXYZI>());
            pcl::transformPointCloud(*xyz_cloud, *transformedTargetCloudKalman, kalmanTransformation);

            // Transform the target cloud using graph optimization pose
            g2o::VertexSE2* lastVertex = dynamic_cast<g2o::VertexSE2*>(optimizer.vertex(vertex_id - 1));
            g2o::SE2 graphPoseSE2 = lastVertex->estimate();
            Eigen::Matrix4f graphTransformation = Eigen::Matrix4f::Identity();
            graphTransformation.block<2,2>(0,0) = graphPoseSE2.rotation().toRotationMatrix().cast<float>();
            graphTransformation(0,3) = graphPoseSE2.translation()[0];
            graphTransformation(1,3) = graphPoseSE2.translation()[1];

            pcl::PointCloud<pcl::PointXYZI>::Ptr transformedTargetCloudGraph(new pcl::PointCloud<pcl::PointXYZI>());
            pcl::transformPointCloud(*xyz_cloud, *transformedTargetCloudGraph, graphTransformation);

            // Accumulate the transformed clouds
            *gicpTransformedCloud += *transformedTargetCloudGICP;
            *kalmanTransformedCloud += *transformedTargetCloudKalman;
            *graphTransformedCloud += *transformedTargetCloudGraph;

            // Update the visualizer
            viewer->removeAllPointClouds();
            pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZI> gicpColorHandler(gicpTransformedCloud, 255, 0, 0); // Red
            viewer->addPointCloud<pcl::PointXYZI>(gicpTransformedCloud, gicpColorHandler, "GICP Cloud");

            pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZI> kalmanColorHandler(kalmanTransformedCloud, 0, 255, 0); // Green
            viewer->addPointCloud<pcl::PointXYZI>(kalmanTransformedCloud, kalmanColorHandler, "Kalman Cloud");

            pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZI> graphColorHandler(graphTransformedCloud, 0, 0, 255); // Blue
            viewer->addPointCloud<pcl::PointXYZI>(graphTransformedCloud, graphColorHandler, "Graph Cloud");
