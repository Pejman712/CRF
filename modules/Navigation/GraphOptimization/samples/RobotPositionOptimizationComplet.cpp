...
int main(int argc, char** argv) {
    ...
    std::string configFileCamera(sensorConfigFile_);
    ...
    std::string configFileNoise(graphConfigFile_);
    ...
    boost::optional<crf::utility::types::TaskPose> boostRobotPosition;
    boost::optional<crf::utility::types::TaskPose> boostPreviousRobotPosition;
    ...
    while ((id_vertex < maxIterations_) && (gSignalStatus != SIGKILL)) {
        ...
        if ((distance > abs(threshold)) || (id_vertex == 0)) {
            ...
            *inPointCloud = crf::sensors::rgbdcamera::RGBDUtils::getPointCloud(color, depth,
            ...
            std::cout << "1 - Point adquisition execution time: " <<
    ...
    // Save PC
    std::string savePointCloudName {"../modules/Applications/GraphOptimization/results/"};
    savePointCloudName += resultName_;
    pcl::io::savePCDFileASCII(savePointCloudName, *cloud_);
