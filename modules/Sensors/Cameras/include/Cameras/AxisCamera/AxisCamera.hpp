...
class AxisCamera : public ICamera {
 public:
    AxisCamera() = delete;
    AxisCamera(const std::string& httpAddress, const std::string& username = "",
    ...
    bool setResolutionAndFramerate(const cv::Size& resolution, int fps) override;
    ...
    std::optional<FocusModes> getFocusMode() override;
