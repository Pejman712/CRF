...
AxisCamera::AxisCamera(const std::string& httpAddress, const std::string& username,
    const std::string& password) :
    logger_("AxisCamera"),
    cameraAddress_(httpAddress),
    username_(username),
    password_(password),
    streamOn_(false),
    initialized_(false),
...
bool AxisCamera::initialize() {
    ...
    try {
        response = httpRequest(
            "/axis-cgi/param.cgi?action=list&group=root.Properties.API.HTTP.Version");
    ...
    try {
        response = httpRequest("/axis-cgi/imagesize.cgi?camera=1");
    } catch (const std::exception& ex) {
        logger_->error("Could not connect to camera: {}", ex.what());
        return false;
    }
    auto resVec = split(response, '\n');
    currentResolution_ = cv::Size(std::stoi(resVec[0].substr(resVec[0].find('=')+1)),
...
bool AxisCamera::setResolutionAndFramerate(const cv::Size& resolution, int fps) {
    logger_->debug("setResolutionAndFramerate()");
    if (!initialized_) {
        logger_->warn("Not initialized");
        return false;
    }
    ...
    if (!resolutionExists) {
     ...
     if (streamOn_) {
        if (!stopFrameStream()) {
            logger_->warn("Could not stop stream to change resolution and framerate");
            return false;
        }

        if (!startFrameStream()) {
            logger_->warn("Could not restart the stream after changing resolution and framerate");
            return false;
        }
    }
...
std::optional<cv::Size> AxisCamera::getResolution() {
...
std::optional<int> AxisCamera::getFramerate() {
...
std::optional<float> AxisCamera::getZoom() {
...
std::optional<float> AxisCamera::getPan() {
...
std::optional<float> AxisCamera::getTilt() {
    logger_->debug("getPan()");
    if (!initialized_) {
        logger_->warn("Not initialized");
        return std::nullopt;
...
std::optional<float> AxisCamera::getExposure() {
    logger_->debug("getExposure()");
    logger_->warn("Not available");
    return std::nullopt;
}

std::optional<float> AxisCamera::getShutterSpeed() {
    logger_->debug("getShutterSpeed()");
    logger_->warn("Not available");
    return std::nullopt;
}

std::optional<ICamera::FocusModes> AxisCamera::getFocusMode() {
    ...
    std::string response;
    try {
        response = httpRequest("/axis-cgi/com/ptz.cgi?query=position");
    } catch (const std::exception& ex) {
        return std::nullopt;
    }
...
std::optional<float> AxisCamera::getFocus() {
...
std::optional<int> AxisCamera::getISO() {
...
std::vector<cv::Size> AxisCamera::getAvailableResolutions() {
    ...
    try {
        response = httpRequest(
            "/axis-cgi/param.cgi?action=list&group=root.Properties.Image.Resolution");
    } catch (const std::exception& ex) {
        return resolutions;
    }
...
std::vector<int> AxisCamera::getAvailableFramerates(const cv::Size& resolution) {
    // TODO(alavrog): This function has to be implemented yet.
