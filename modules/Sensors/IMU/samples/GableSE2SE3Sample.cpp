...
int main(int argc, char* argv[]) {
    ...
    crf::sensors::imu::GableSE2SE3 gableIMU(masterEthercatImu, deviceId);
    gableIMU.initialize();
    ...
    if (info) {
        ...
        std::cout << "FilterProfile: " << info.value().FilterProfile.value() << "\n";
    ...
    gableIMU.enableAHS();
    gableIMU.disableAHS();
    gableIMU.setFilter(crf::sensors::imu::FilterProfile::General);
    info = gableIMU.getInfo();
    std::cout << "New FilterProfile: " << info.value().FilterProfile.value() << "\n";

    while (true) {
        crf::sensors::imu::IMUSignals signal = gableIMU.getSignal();
        crf::expected<std::array<double, 4>> quaternion = signal.quaternion;
        crf::expected<std::array<double, 3>> eulerZYX = signal.eulerZYX;
        ...
        if (quaternion) {
            for (int i=0; i < 4; i++) {
                std::cout << signal.quaternion.value()[i] << " ";
            }
        } else {
            crf::ResponseCode errorOr = quaternion.get_response();
            std::cout << errorOr;
        }

        std::cout << " ]"<< std::endl << "Orientation (Eul): [ ";
        if (eulerZYX) {
            for (int i=0; i < 3; i++) {
                std::cout << signal.eulerZYX.value()[i] << " ";
            }
        } else {
            crf::ResponseCode errorEul = eulerZYX.get_response();
            std::cout << errorEul;
        }
