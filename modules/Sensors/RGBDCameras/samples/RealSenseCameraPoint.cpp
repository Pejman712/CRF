...
using crf::sensors::rgbdcameras::D405RealSenseCamera;
...
int main(int argc, char* argv[]) {
    ...
    desc.add_options()
        ...
        ("d405", "Specify if the camera is a D405");
    ...
    std::shared_ptr<IRGBDCamera> camera;
    if (vm.count("d405")) {
        camera = std::make_shared<D405RealSenseCamera>(vm["serial_number"].as<std::string>());
    } else {
        camera = std::make_shared<RealSenseCamera>(vm["serial_number"].as<std::string>());
    }
