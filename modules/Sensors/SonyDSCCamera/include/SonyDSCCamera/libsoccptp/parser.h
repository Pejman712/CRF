...
public:
	...
	uint16_t DataType; //!< This field identifies the Datatype Code of the property.
	uint8_t GetSet; //!< This field indicates whether the property is read-only or read-write.
	uint8_t IsEnable; //!< This field indicates whether the property is valid, invalid or DispOnly.
 ...
 */
template <typename T> class DataTypeInteger : public SDIDevicePropInfoDataset {
	friend class SDIDevicePropInfoDataset;
public:
	T DefaultValue; //!< This field identifies the value of the factory default setting for the property.
	T CurrentValue; //!< This field identifies the current value of the property.
	T MinimumValue; //!< Minimum value of property supported by the device.\n If the FormFlag is Range-Form, this is valid.
	T MaximumValue; //!< Maximum value of property supported by the device.\n If the FormFlag is Range-Form, this is valid.
	T StepSize; //!< The steps of the supported value like MinimumValue + N x StepSize which is less than or equal to MaximumValue.\n If the FormFlag is Range-Form, this is valid.
	uint16_t NumOfValues; //!< This field indicates the number of values of the particular property supported by the device.\n If the FormFlag is Enumeration-Form, this is valid.
 ...
 */
template <typename T> class DataTypeArray : public SDIDevicePropInfoDataset {
	friend class SDIDevicePropInfoDataset;
public:
	uint32_t NumOfDefaultValues; //!< Represents the total number of elements of the factory default setting contained in the array.
	uint32_t NumOfCurrentValues; //!< Represents the total number of elements of the current value contained in the array.
	T* DefaultValues; //!< This field is the array of the factory default setting value.
...
protected:
	...
	/**
	 * @brief In this parser the form section is not supported.\n If the FormFlag is not None, the parsed result of the all following datasets is corrupt.
	 */
	virtual void parseForm(char **data);
	DataTypeArray() {};
};
...
class DataTypeSTR : public SDIDevicePropInfoDataset {
	friend class SDIDevicePropInfoDataset;
public:
	char *DefaultValue; //!< This field identifies the value of the factory default setting for the property.
	char *CurrentValue; //!< This field identifies the current value of the property.
...
public:
	...
	/**
	 * @brief gets the specified SDIDevicePropInfoDataset as \em device_property_code.
	 * @param device_property_code a device property code.
	 * @return the instance of SDIDevicePropInfoDataset.\n It should be casted to a suitable DataType in accordance with the member variable of DataType.
	 * @see DataTypeInteger
	 * @see DataTypeArray
	 * @see DataTypeSTR
	 */
...
public:
	...
	/**
	 * @brief returns the pointer of the LiveView image to be able to be decoded as JPEG.
	 */
	uint8_t *get();
};
...
public:
	uint32_t num; //<! Represents the total number of elements contained in the array.
