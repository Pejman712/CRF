...
public :
    GenericBulkContainer(uint32_t length, uint16_t type, uint16_t code, uint32_t transaction_id, uint32_t size){
    }
...
private :
    ...
    int senddata(uint16_t code, uint32_t* parameters, uint8_t num, void* data, unsigned int size);
