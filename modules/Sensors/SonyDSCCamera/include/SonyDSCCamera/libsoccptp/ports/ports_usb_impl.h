...
private :
    ...
    static int LIBUSB_CALL hotplug_callback_entry(libusb_context *ctx, libusb_device *device, libusb_hotplug_event event, void *user_data);
...
private :
    struct libusb_config_descriptor* find_configurations(libusb_device& device, int index = 0);
    const struct libusb_interface_descriptor* find_altsetting_by_class_code(libusb_config_descriptor& config_descriptor, libusb_class_code code);
    bool configure_endpoint(const struct libusb_interface_descriptor& altsetting, int& inep, int& outep, int& intep);
