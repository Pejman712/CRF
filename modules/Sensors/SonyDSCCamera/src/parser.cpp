...
#define FORMAT(__v, __t)    ((__v) & (0xFFFFFFFFFFFFFFFF >> (64 - sizeof(__t) * 8)))
...
SDIDevicePropInfoDataset *SDIDevicePropInfoDataset::parse(void *data) {
	char *_data = (char *)data;
	DevicePropertyCode = *(uint16_t*)_data; _data += sizeof(uint16_t);
	DataType = *(uint16_t*)_data; _data += sizeof(uint16_t);
	GetSet = *(uint8_t*)_data; _data += sizeof(uint8_t);
	IsEnable = *(uint8_t*)_data; _data += sizeof(uint8_t);
	parseValues(&_data);
	FormFlag = *(uint8_t*)_data; _data += sizeof(uint8_t);
	parseForm(&_data);
...
void DataTypeInteger<T>::parseValues(char **data) {
	DefaultValue = *(T*)*data; *data += sizeof(T);
	CurrentValue = *(T*)*data; *data += sizeof(T);
}
...
void DataTypeInteger<T>::parseForm(char **data) {
	...
	case 0x01: // Range-Form
		MinimumValue = *(T*)*data; *data += sizeof(T);
		MaximumValue = *(T*)*data; *data += sizeof(T);
		StepSize = *(T*)*data; *data += sizeof(T);
		break;
	case 0x02: // Enumeration-Form
		NumOfValues = *(uint16_t *)*data; *data += sizeof(uint16_t);
		Values = new T[NumOfValues];
		for (uint16_t i = 0; i < NumOfValues; i++) {
			Values[i] = *(T*)*data; *data += sizeof(T);
		}
	}
}
...
void DataTypeArray<T>::parseValues(char **data) {
	char **_data = (char **)data;
	NumOfDefaultValues = *(uint32_t *)*_data; *_data += sizeof(uint32_t);
	DefaultValues = new T[NumOfDefaultValues];
	for (uint32_t i = 0; i < NumOfDefaultValues; i++) {
		DefaultValues[i] = *(T*)*_data; *_data += sizeof(T);
	}
	
	NumOfCurrentValues = *(uint32_t *)*_data; *_data += sizeof(uint32_t);
	CurrentValues = new T[NumOfCurrentValues];
	for (uint32_t i = 0; i < NumOfCurrentValues; i++) {
		CurrentValues[i] = *(T*)*_data; *_data += sizeof(T);
	}
}
...
void DataTypeArray<T>::parseForm(char **data) {
	if (0x00 != FormFlag) {
		printf("DevicePropertyCode: 0x%04X: FormFlag is not 0, this is not supported\n", DevicePropertyCode);
	}
}
...
void DataTypeArray<T>::toString(std::string& str) {
	...
	for (uint32_t i = 0; i < NumOfDefaultValues; i++) {
		strsprintf(str, "  dataset   DefaultValue: %0*llX\n", sizeof(T) * 2, FORMAT(DefaultValues[i], T));
	}
	strsprintf(str, "  dataset NumOfCurrentValues: %u\n", NumOfCurrentValues);
	for (uint32_t i = 0; i < NumOfCurrentValues; i++) {
		strsprintf(str, "  dataset   CurrentValue: %0*llX\n", sizeof(T) * 2, FORMAT(CurrentValues[i], T));
	}
}
...
void DataTypeSTR::parseValues(char **data) {
	uint8_t len;
	// DefaultValue
	length_DefaultValue = len = *(uint8_t*)*data; *data += sizeof(uint8_t);
	...
	*data += sizeof(uint16_t) * len;
	...
	// CurrentValue
	length_CurrentValue = len = *(uint8_t*)*data; *data += sizeof(uint8_t);
...
SDIDevicePropInfoDatasetArray::SDIDevicePropInfoDatasetArray(void *data) {
	...
	num = *(uint64_t*)_data; _data += sizeof(uint64_t);
	for (uint64_t i = 0; i < num; i++) {
		SDIDevicePropInfoDataset *d = SDIDevicePropInfoDataset::create((void *)_data);
...
SDIDevicePropInfoDataset *SDIDevicePropInfoDatasetArray::get(uint16_t device_property_code) {
	SDIDevicePropInfoDataset *ret = NULL;
...
LiveViewImage::LiveViewImage(void *data) {
	...
	org_data = data;
	offset = *(uint32_t*)_data; _data += sizeof(uint32_t);
	_size = *(uint32_t*)_data; _data += sizeof(uint32_t);
...
uint32_t LiveViewImage::size() {
	return _size;
}

uint8_t *LiveViewImage::get() {
	return data;
}
...
SimpleArray<T>::SimpleArray(void *data) {
	char *_data = (char *)data;
	num = *(uint32_t *)_data; _data += sizeof(uint32_t);
	values = new T[num];
	for (uint32_t i = 0; i < num; i++) {
		values[i] = *(T *)_data; _data += sizeof(T);
	}
}
