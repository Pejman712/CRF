...
ports_ptp_impl::ports_ptp_impl(int busn, int devn, uint32_t session_id, uint32_t transaction_id, ports_usb* usb)
...
int ports_ptp_impl::senddata(uint16_t code, uint32_t* parameters, uint8_t num, void* data, unsigned int size){
    int ret;
    GenericBulkContainerHeader* header;
    uint32_t* payload;
...
int ports_ptp_impl::getdata(void** data, uint32_t& size){
    ...
    if(header->type != 0x0002){
        ...
        *data = cp;
        ...
        size = (actual > sizeof(GenericBulkContainerHeader)) ? actual - sizeof(GenericBulkContainerHeader) : 0;
    }
...
int ports_ptp_impl::getresp(com::sony::imaging::remote::Container& response){
    ...
    if(header->type != 0x0003){
        ...
        nparam = (header->length - sizeof(GenericBulkContainerHeader))/sizeof(uint32_t);
