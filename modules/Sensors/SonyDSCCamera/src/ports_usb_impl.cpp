...
int ports_usb_impl::open(){
    ...
    target_device->get_settings(configuration_value, interface_number, alternate_setting);
    ...
    if(ret ==  LIBUSB_SUCCESS){
        libusb_get_string_descriptor_ascii(device_handle, current_device.iManufacturer, current_device.ascii_Manufacturer, sizeof(current_device.ascii_Manufacturer));
        libusb_get_string_descriptor_ascii(device_handle, current_device.iProduct, current_device.ascii_Product, sizeof(current_device.ascii_Product));
        libusb_get_string_descriptor_ascii(device_handle, current_device.iSerialNumber, current_device.ascii_SerialNumber, sizeof(current_device.ascii_SerialNumber));
    }
    ...
    if(libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG) != 0){
        ret = libusb_hotplug_register_callback(context,
                                               ...
                                               current_device.idVendor, current_device.idProduct, LIBUSB_HOTPLUG_MATCH_ANY,
...
void ports_usb_impl::set_hotplug_callback(socc_hotplug_callback_func_t callback_func, void* vp){
...
int LIBUSB_CALL ports_usb_impl::hotplug_callback_entry(libusb_context *ctx, libusb_device *device, libusb_hotplug_event event, void *user_data){
...
ports_usb_impl_target_device::ports_usb_impl_target_device(libusb_context* context) : context(context), device(NULL), configuration_value(-1), interface_number(-1), alternate_setting(-1), inep(-1), outep(-1), intep(-1){
}
...
libusb_device* ports_usb_impl_target_device::find_device(int busn, int devn, libusb_class_code code){
    int count;
    libusb_device** devs = NULL;
    bool found = false;
    ...
    for(int i=0; i<count && found==false; i++){
        ...
        while(found==false) {
            ...
            if((busn!=0 && devn!=0) && (busn!=current_device.busn || devn!=current_device.devn)){
                continue;
            }
            
            altsetting = find_altsetting_by_class_code(*config_descriptor, LIBUSB_CLASS_PTP);
            if(altsetting != NULL){
...
void ports_usb_impl_target_device::get_settings(int& config, int& inter, int& alt){
    config = configuration_value;
    inter = interface_number;
    alt = alternate_setting;
}
void ports_usb_impl_target_device::get_endpoint(int& inep, int& outep, int& intep){
    inep = this->inep;
    outep = this->outep;
    intep = this->intep;
}

const struct libusb_interface_descriptor* ports_usb_impl_target_device::find_altsetting_by_class_code(libusb_config_descriptor& config_descriptor, libusb_class_code code){
    const struct libusb_interface_descriptor* altsetting = NULL;
...
struct libusb_config_descriptor* ports_usb_impl_target_device::find_configurations(libusb_device& device, int index){
    struct libusb_config_descriptor* config_descriptor = NULL;
...
bool ports_usb_impl_target_device::configure_endpoint(const struct libusb_interface_descriptor& altsetting, int& inep, int& outep, int& intep){
    inep = -1;
    outep = -1;
    intep = -1;
    for(int i=0; i < altsetting.bNumEndpoints; i++){
        if((altsetting.endpoint[i].bmAttributes & LIBUSB_TRANSFER_TYPE_MASK) ==  LIBUSB_TRANSFER_TYPE_BULK){
        ...
        } else if((altsetting.endpoint[i].bmAttributes & LIBUSB_TRANSFER_TYPE_MASK) == LIBUSB_TRANSFER_TYPE_INTERRUPT){
            if(altsetting.endpoint[i].bEndpointAddress & LIBUSB_ENDPOINT_IN){
