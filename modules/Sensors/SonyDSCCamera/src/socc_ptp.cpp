...
void socc_ptp::set_hotplug_callback(socc_hotplug_callback_func_t callback_func, void* vp){
...
void socc_ptp::dispose_data(void** data){
    ptp->dispose_data(data);
}

int socc_ptp::clear_halt(int what){
    return usb->clear_halt(what);
}
