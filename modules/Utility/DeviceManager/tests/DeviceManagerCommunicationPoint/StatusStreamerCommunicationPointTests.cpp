...
class StatusStreamerCommunicationPointShould : public ::testing::Test {
 protected:
    ...
    std::unique_ptr<crf::utility::devicemanager::StatusStreamerCommunicationPoint>
    openSocketsCounter = 0;
};
...
TEST_F(StatusStreamerCommunicationPointShould, checkDeviceManagerDeinitialization) {
    sut_.reset(new crf::utility::devicemanager::StatusStreamerCommunicationPoint(
        packetSocket_, deviceMock_));
    ASSERT_TRUE(sut_->initialize());
    openSocketsCounter = 0;
...
TEST_F(StatusStreamerCommunicationPointShould, correctlyStartStatusStreamWithDesiredFrequencyIfModeIsFrequency) {  // NOLINT
    sut_.reset(new crf::utility::devicemanager::StatusStreamerCommunicationPoint(
        packetSocket_, deviceMock_));
    ...
    crf::communication::datapackets::JSONPacket startStreamJSON;
    startStreamJSON.data["command"] = "startStreamStatus";
    startStreamJSON.data["mode"] = crf::utility::devicemanager::StatusStreamerMode::Frequency;
    startStreamJSON.data["frequency"] = 25.0f;
...
TEST_F(StatusStreamerCommunicationPointShould, sendOnlyOneStreamStatusIfAllAreEqualInModeDifferential) {  // NOLINT
    sut_.reset(new crf::utility::devicemanager::StatusStreamerCommunicationPoint(
        packetSocket_, deviceMock_));
    ...
    crf::communication::datapackets::JSONPacket startStreamJSON;
    startStreamJSON.data["command"] = "startStreamStatus";
    startStreamJSON.data["mode"] = crf::utility::devicemanager::StatusStreamerMode::Differential;
    startStreamJSON.data["frequency"] = 25.0f;
...
TEST_F(StatusStreamerCommunicationPointShould, supportsIdParameter) {
    sut_.reset(new crf::utility::devicemanager::StatusStreamerCommunicationPoint(
        packetSocket_, deviceMock_));
    ...
    crf::communication::datapackets::JSONPacket json;
    json.data["command"] = "getStatus";
    json.data["id"] = "a string identifier";
    writePacket(json);

    auto response = readJsonPacket();
    ASSERT_TRUE(response);
    ASSERT_EQ(response.value().data["id"], "a string identifier");
    ASSERT_EQ(response.value().data["replyCommand"], "getStatus");
    ASSERT_EQ(response.value().data["message"], statusJSON_);

    json.data["command"] = "invalid";
    json.data["id"] = 1234;
    writePacket(json);
    response = readJsonPacket();
    ASSERT_TRUE(response);
    ASSERT_EQ(response.value().data["id"], 1234);
    ASSERT_EQ(response.value().data["command"], "reply");
    ASSERT_EQ(response.value().data["replyCommand"], "error");
...
TEST_F(StatusStreamerCommunicationPointShould, alllowToConfigureAHeartbeat) {
    sut_.reset(new crf::utility::devicemanager::StatusStreamerCommunicationPoint(
        packetSocket_, deviceMock_));
    ...
    crf::communication::datapackets::JSONPacket json;
    json.data["command"] = "startHeartbeat";
    json.data["period"] = 1;
    writePacket(json);
    ...
    ASSERT_TRUE(response);
    ASSERT_EQ(response.value().data["command"], "reply");
    ASSERT_EQ(response.value().data["replyCommand"], "startHeartbeat");
    ASSERT_EQ(response.value().data["message"], true);

    json.data["command"] = "heartbeat";
    writePacket(json);
    response = readJsonPacket();
    ASSERT_TRUE(response);
    ASSERT_EQ(response.value().data["command"], "reply");
    ASSERT_EQ(response.value().data["replyCommand"], "heartbeat");
    ASSERT_EQ(response.value().data["message"], true);

    json.data["command"] = "stopHeartbeat";
    writePacket(json);
    response = readJsonPacket();
    ASSERT_TRUE(response);
    ASSERT_EQ(response.value().data["command"], "reply");
    ASSERT_EQ(response.value().data["replyCommand"], "stopHeartbeat");
    ASSERT_EQ(response.value().data["message"], true);

    ASSERT_TRUE(sut_->deinitialize());
}

TEST_F(StatusStreamerCommunicationPointShould, notAllowWrongInputsInHeartbeat) {
    sut_.reset(new crf::utility::devicemanager::StatusStreamerCommunicationPoint(
        packetSocket_, deviceMock_));
    ...
    ASSERT_EQ(response.value().data["command"], "reply");
    ASSERT_EQ(response.value().data["replyCommand"], "error");
    ASSERT_EQ(response.value().data["message"], "Heartbeat period not valid");
    ...
    ASSERT_EQ(response.value().data["command"], "reply");
    ASSERT_EQ(response.value().data["replyCommand"], "error");
    ASSERT_EQ(response.value().data["message"], "Heartbeat has not been configured");
