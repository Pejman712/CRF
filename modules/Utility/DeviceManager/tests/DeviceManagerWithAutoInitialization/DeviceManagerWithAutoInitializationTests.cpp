...
TEST_F(DeviceManagerWithAutoInitializationShould, getStatusAccordingToInitialization) {
    std::chrono::milliseconds initializationTimeout(200);
...
TEST_F(DeviceManagerWithAutoInitializationShould, notFailIfTimeoutIs0) {
    std::chrono::milliseconds initializationTimeout(0);
    ASSERT_NO_THROW(sut_.reset(new crf::utility::devicemanager::DeviceManagerWithAutoInitialization(  // NOLINT
        deviceMock_, initializationTimeout)));
}
