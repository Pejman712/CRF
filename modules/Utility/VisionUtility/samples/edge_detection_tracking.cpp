...
int main(int argc, char** argv) {
    if (argc != 4) {
        std::cerr << "Usage: " << argv[0] << " <path-to-pcd-folder> <percent> <ransac-threshold>" << std::endl;
        return -1;
    }

    std::string pcd_folder = argv[1];
    float percent = std::stof(argv[2]);  // Convert the argument to a float
    float ransac_threshold = std::stof(argv[3]);

    // Create output CSV file
    std::ofstream csv_file("tracking_result.csv");
    csv_file << "Scan1,Scan2,TrackingConsistency,AverageDistance,ProcessingTime\n"; // Header for CSV file
    ...
    if (pcd_files.size() < 2) {
        std::cerr << "Not enough PCD files in the folder to perform comparison." << std::endl;
        return -1;
    }

    // Iterate through consecutive scans
    for (size_t i = 1; i < pcd_files.size(); ++i) {
        std::cout << "Comparing scan " << i - 1 << " (" << pcd_files[i - 1] << ") with scan " << i << " (" << pcd_files[i] << ")." << std::endl;
        ...
        if (pcl::io::loadPCDFile<pcl::PointXYZ>(pcd_files[i - 1], *previous_cloud) == -1) {
            std::cerr << "Failed to load PCD file: " << pcd_files[i - 1] << std::endl;
            continue;
        }

        // Load the current scan
        pcl::PointCloud<pcl::PointXYZ>::Ptr current_cloud(new pcl::PointCloud<pcl::PointXYZ>);
        if (pcl::io::loadPCDFile<pcl::PointXYZ>(pcd_files[i], *current_cloud) == -1) {
            std::cerr << "Failed to load PCD file: " << pcd_files[i] << std::endl;
            continue;
        }

        std::cout << "Loaded point clouds with " << previous_cloud->points.size() << " and " << current_cloud->points.size() << " points." << std::endl;

        // Call the edge sharpness function for both previous and current scans
        unsigned int kNeighbour = 100;
        pcl::PointCloud<crf::utility::visionutility::pointcloud::pointtypes::PointXYZS>::Ptr sharpness_previous =
            crf::utility::visionutility::pointcloud::edge::computeEigenSharpness<pcl::PointXYZ>(previous_cloud, kNeighbour);
        pcl::PointCloud<crf::utility::visionutility::pointcloud::pointtypes::PointXYZS>::Ptr sharpness_current =
            crf::utility::visionutility::pointcloud::edge::computeEigenSharpness<pcl::PointXYZ>(current_cloud, kNeighbour);

        if (!sharpness_previous || sharpness_previous->empty() || !sharpness_current || sharpness_current->empty()) {
            std::cerr << "No sharpness data computed!" << std::endl;
            continue;
        }

        // Filter the sharpness to show only the top N%
        auto filtered_previous = filterTopSharpness(sharpness_previous, percent);
        auto filtered_current = filterTopSharpness(sharpness_current, percent);

        std::cout << "Filtered point clouds to top " << percent << "% by sharpness." << std::endl;

        // Extract XYZ from the sharpness filtered clouds
        pcl::PointCloud<pcl::PointXYZ>::Ptr xyz_filtered_previous = extractXYZ(filtered_previous);
        pcl::PointCloud<pcl::PointXYZ>::Ptr xyz_filtered_current = extractXYZ(filtered_current);

        // Calculate tracking consistency between the previous and current scans using the XYZ points
        float tracking_consistency = calculateTrackingConsistency(xyz_filtered_previous, xyz_filtered_current, ransac_threshold);
        std::cout << "Tracking consistency: " << tracking_consistency << "%" << std::endl;

        // Calculate average distance between closest points
        float average_distance = calculateAverageDistanceBetweenClosestPoints(previous_cloud, current_cloud);
        std::cout << "Average distance between closest points: " << average_distance << std::endl;

        // Measure the end time and calculate processing duration
        auto end_time = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> processing_time = end_time - start_time;
        std::cout << "Processing time: " << processing_time.count() << " seconds." << std::endl;

        // Write results to CSV
        csv_file << pcd_files[i - 1] << "," << pcd_files[i] << "," << tracking_consistency << "," << average_distance << "," << processing_time.count() << "\n";

        // Optionally visualize sharpness (if needed)
        //visualizeSharpness(current_cloud, filtered_current);
    }
