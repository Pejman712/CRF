...
#include "DirectSparseOdometry/util/globalCalib.hpp"
...
public:
	ImageFolderReader(std::string path, std::string calibFile, std::string gammaFile, std::string vignetteFile)
	{
		...
		widthOrg = undistort_->getOriginalSize()[0];
		heightOrg = undistort_->getOriginalSize()[1];
		width=undistort_->getSize()[0];
		height=undistort_->getSize()[1];
	...
	{
		return undistort_->getOriginalParameter().cast<float>();
	}
	Eigen::Vector2i getOriginalDimensions()
	{
		return  undistort_->getOriginalSize();
	}
	...
	{
		if(undistort_ == nullptr || undistort_->photometricUndist==0) return 0;
		return undistort_->photometricUndist->getG();
	}


	// undistorter. [0] always exists, [1-2] only when MT is enabled.
	std::unique_ptr<Undistort> undistort_;
private:
	...
	ImageAndExposure* getImage_internal(int id, int unused)
	{
		...
		ImageAndExposure* ret2 = undistort_->undistort<unsigned char>(
				minimg,
