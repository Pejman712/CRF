...
public:
	...
	std::unique_ptr<ImageAndExposure> output;

	float* getG() {if(!valid) return 0; else return G;};
private:
    float G[256*256];
    int GDepth;
	std::vector<float> vignetteMap;
	std::vector<float> vignetteMapInv;
	int w,h;
	bool valid;
};
...
public:
	...
	static std::unique_ptr<Undistort> getUndistorterForFile(std::string configFilename, std::string gammaFilename, std::string vignetteFilename);
