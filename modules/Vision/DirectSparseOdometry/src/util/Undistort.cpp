...
#include <Eigen/Core>
#include <iterator>
#include "DirectSparseOdometry/util/settings.hpp"
#include "DirectSparseOdometry/util/globalFuncs.hpp"
#include "DirectSparseOdometry/IOWrapper/ImageDisplay.hpp"
#include "DirectSparseOdometry/IOWrapper/ImageRW.hpp"
		...
		int w_, int h_)
{
	...
	w = w_;
	h = h_;
	output.reset(new ImageAndExposure(w,h));
	...
	if (!f.good())
	{
		...
		throw std::runtime_error("PhotometricUndistorter: Could not open file!");
	}
...

	{
        ...
        if(GDepth < 256)
        {
            ...
            throw std::runtime_error("PhotometricUndistorter: invalid format!");
        }
        ...
        for(int i=0;i<GDepth-1;i++)
		{
			if(G[i+1] <= G[i])
			{
				...
				throw std::runtime_error("PhotometricUndistorter: G invalid!");
			}
		}
	...
	vignetteMap.resize(w*h);
	vignetteMapInv.resize(w*h);
	...
	if(vm16 != 0)
	{
		if(vm16->w != w ||vm16->h != h)
		{
			...
			if(vm8!=0) delete vm8;
			throw std::runtime_error("PhotometricUndistorter: Invalid vignette image size!");
		}
	...
	else
	{
		...
		if(vm16!=0) delete vm16;
		if(vm8!=0) delete vm8;
		throw std::runtime_error("PhotometricUndistorter: Invalid vignette image!");
	}
...
PhotometricUndistorter::~PhotometricUndistorter()
{
	printf("PhotometricUndistorter DTor\n");
}
...
void PhotometricUndistorter::processFrame(T* image_in, float exposure_time, float factor)
{
	...
	// std::cout << "yay! I made it into PhotometricUndistorter::processFrame!" << std::endl;
...
Undistort::~Undistort()
{
	printf("Undistort DTor\n");
	if(remapX != 0) delete[] remapX;
	if(remapY != 0) delete[] remapY;
}

std::unique_ptr<Undistort> Undistort::getUndistorterForFile(std::string configFilename, std::string gammaFilename, std::string vignetteFilename)
{
			...
			&ic[4], &ic[5], &ic[6], &ic[7]) == 8)
	{
        ...
        u.reset(new UndistortRadTan(configFilename.c_str(), true));
			...
			&ic[0], &ic[1], &ic[2], &ic[3], &ic[4]) == 5)
	{
		if(ic[4]==0)
		{
            ...
            u.reset(new UndistortPinhole(configFilename.c_str(), true));
			if(!u->isValid()) { return nullptr; }
		}
		else
		{
			printf("found ATAN camera model, building rectifier.\n");
            u.reset(new UndistortFOV(configFilename.c_str(), true));
			if(!u->isValid()) { return nullptr; }
		}
	}
    ...
    // clean model selection implementation.
            ...
            &ic[4], &ic[5], &ic[6], &ic[7]) == 8)
    {
        u.reset(new UndistortKB(configFilename.c_str(), false));
        if(!u->isValid()) { return nullptr; }
    }
            ...
            &ic[4], &ic[5], &ic[6], &ic[7]) == 8)
    {
        u.reset(new UndistortRadTan(configFilename.c_str(), false));
        if(!u->isValid()) { return nullptr; }
    }
            ...
            &ic[4], &ic[5], &ic[6], &ic[7]) == 8)
    {
        u.reset(new UndistortEquidistant(configFilename.c_str(), false));
        if(!u->isValid()) { return nullptr; }
    }
            ...
            &ic[4]) == 5)
    {
        u.reset(new UndistortFOV(configFilename.c_str(), false));
        if(!u->isValid()) { return nullptr; }
    }
            ...
            &ic[4]) == 5)
    {
        u.reset(new UndistortPinhole(configFilename.c_str(), false));
        if(!u->isValid()) { return nullptr; }
    }
...
{
	photometricUndist.reset(new PhotometricUndistorter(
		file, noiseImage, vignetteImage,getOriginalSize()[0], getOriginalSize()[1]));
}
