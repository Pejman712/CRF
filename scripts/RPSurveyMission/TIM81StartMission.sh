...
Instructions() {
    ...
    echo "      RP Survey with the TIM81. These are: "
    echo ""
...
TIM() {
    sudo -b -E ./../bin/TIMS300Point --protocol tcp --port 4000 --configuration ../modules/Actuators/TIM/config/TIM81.json
}

TIMRobotArmAndStabilizer() {
    sudo ldconfig /opt/JACO-SDK/API
    sleep 1
    sudo -b -E ./../bin/TIMArmWagonActuatorsCommunicationPoint --ethercat_port enp5s0 --port_stabilizer 3000 --port_shielding 3001 --port_arm 3002 --configuration ../modules/Actuators/TIMArm/config/TIM81RobotArm.json
}
...
EndEffectorCamera() {
    sudo -b -E ./../bin/UVCCameraPoint --device_name /dev/video0 --protocol tcp --port 8080
}
...
RPSurveyMission() {
    sudo -b -E ./../bin/RPSurveyLHCPoint --missionPort 5000 --parametersPath ../modules/Applications/MissionManager/config/RPSurveyLHC/missionParametersTIM81Arm.json
}
...
StartPoints() {
    ...
    echo "     - TIM81                |  TCP Port 4000"
    echo "     - TIM81 TIM Arm        |  TCP Port 3002"
    echo "     - RP Sensor            |  TCP Port 8000"
    echo "     - Oxygen Sensor        |  TCP Port 8001"
    echo "     - Temperatur Sensor    |  TCP Port 8002"
    echo "     - RP Survey Mission    |  TCP Port 5000"
